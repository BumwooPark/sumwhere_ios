...
class AlertSettingViewController: FormViewController, NVActivityIndicatorViewable{
  ...
  lazy var allAlert = SwitchRow("모든알림", { (row) in
    row.title = "모든알림"
...
//    row.updateCell()
      ...
      log.info("allalert update")
      row.value = self?.pushAlertItem?.isAllTrue()
    })
  
  lazy var match = SwitchRow("동행매칭알림", { (row) in
    row.title = "동행매칭알림"
...
//    self?.update()
  ...
  lazy var friend = SwitchRow("친구요청알림", { (row) in
    row.title = "친구요청알림"
...
//    self?.update()
  ...
  lazy var chat = SwitchRow("채팅알림", { (row) in
    row.title = "채팅알림"
...
//    self?.update()
  ...
  lazy var event = SwitchRow("이벤트알림", { (row) in
    row.title = "이벤트알림"
